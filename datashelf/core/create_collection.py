...
from datashelf.core.metadata import (
    _initialize_collection_metadata,
    _add_collection_to_datashelf_metadata,
)
...
def create_collection(collection_name: str):
    ...
    if datashelf_path is None or not datashelf_path.is_dir():
        raise NotADirectoryError(
            ".datashelf does not exist. Please initialize datashelf with init() before creating a collection."
        )
    else:
        pass
...
def _create_collection(
    collection_path: Union[str, pathlib.PosixPath], collection_name: str
):
    ...
    logger.info(
        f'Collection directory: {collection_name.lower().replace(" ", "_")} and metadata file created.'
    )
    return 0
