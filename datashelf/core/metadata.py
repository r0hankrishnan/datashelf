...
def _initialize_datashelf_metadata(set_dir: str = None):
    ...
    collections = [
        collection for collection in datashelf_path.iterdir() if collection.is_dir()
    ]
...
def _update_datashelf_metadata_collection_files(
    collection_name: str, collection_path: str
):
    current_timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    collection_files = _get_collection_files(collection_name=collection_name)
    collection_file_names = [
        file["name"] for file in collection_files if file["deleted"] == False
    ]
    ...
    for _, collection in enumerate(data["collections"]):
        if collection.get("collection_name") == collection_name.lower().replace(
            " ", "_"
        ):
...
def _edit_datashelf_metadata(collection_name: str, update: dict):
    ...
    if not set(update_keys).issubset(set(["collection_name", "date_created", "files"])):
        raise ValueError(
            "Invalid key detected in update arg. Update can only have the following keys:\n\t-collection_name\n\t-date_created\n\t-files."
        )
    ...
    for _, collection in enumerate(data["collections"]):
        if collection.get("collection_name").lower().replace(
            " ", "_"
...
def _initialize_collection_metadata(collection_name: str):
    ...
    metadata["files"] = [
        {
            "name": f'{collection_name.lower().replace(" ", "_")}_metadata.yaml',
            "hash": "",
            "date_created": current_timestamp,
            "date_last_modified": "",
            "tag": "",
...
def _add_file_to_collection_metadata(
    collection_name: str,
    file_name: str,
    tag: str,
    message: str,
...
def _update_collection_metadata_info(
    collection_name: str, saved_df_file_path: str, new_version: int
):
...
def _edit_collection_metadata(
    collection_name: str, update: dict, config_or_files: Literal["config", "files"]
):
    """_summary_
    ...
    update_keys = [key for key, value in update.items()]
    if not set(update_keys).issubset(
        set(["most_recent_commit", "number_of_files", "message", "name", "tag"])
    ):
        raise ValueError(
            "Invalid key detected in update arg. Update can only have the following:\nFor updating config:\n\t-most_recent_commit\n\t-number_of_files\nFor updating files:\n\t-message\n\t-name\n\t-tag"
        )
    ...
    collection_metadata_path = os.path.join(
        collection_path, f'{collection_name.lower().replace(" ", "_")}_metadata.yaml'
    )
    current_timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    ...
    # Error handling for incorrect value of config_or_files
        ...
        raise ValueError(
            "Invalid argument for config_or_files, must be either 'config' or 'files'."
        )
