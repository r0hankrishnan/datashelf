...
def init_metadata(datashelf_path:Path):
    ...
    config_metadata: FileEntry = {
        "file_hash": config_hash,
        "name": "DataShelf Config",
        "stored_path": "config.yaml",
        "message": "This file holds basic configuration settings for DataShelf.",
        "tag": "",
        "datetime_added": _get_current_timestamp()
    }
    
    metadata["files"].append(config_metadata)
    
    _atomic_write_json(path = Path(metadata_path), obj = metadata)
    
def create_file_entry(file_hash: str, name: str, stored_path: str, message: str, tag: str):
...
def load_metadata(datashelf_path:Path) -> dict:
    ...
    if not metadata_path.exists():
        raise FileNotFoundError(f"Unable to find metadata at {metadata_path}. Your `.datashelf/` directory may be corrupted. Consider reinitializing your DataShelf project.")
...
def _atomic_write_text(path: Path, text:str) -> None:
    ...
    with NamedTemporaryFile("w", dir = str(path.parent), delete = False, encoding = "utf-8") as tempfile:
        tempfile.write(text)
