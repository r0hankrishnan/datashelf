...
from datashelf.core.display import (
    print_success,
    print_error,
    print_message,
    print_table,
...
def init_command(args):
    ...
    """
    try:
        initialized, datashelf_path = init(
            custom_path=args.path
        )  # If no args.path, will default to args.path = None
...
def main():
    ...
    save_parser.add_argument(
        ...
        "--tag",
        type=str,
        help="An optional tag to associate with the saved file.",
    )
    save_parser.add_argument(
        "--on-duplicate",
        type=str,
