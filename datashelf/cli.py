...
def save_from_file_command(args):
    ...
    if not args.file_path:
        args.file_path = (
            input("Enter the path to the file: ")
            .strip()
            .replace('"', "")
            .replace("'", "")
        )
    ...
    if not args.collection_name:
        ...
        answer = questionary.select(
            message="Select a collection to add file to: ", choices=collection_choices
        ).ask()
        if not answer:
            sys.exit(1)
    ...
    if not args.tag:
        ...
        answer = questionary.select(
            message="Select a tag for the file: ", choices=tag_options
        ).ask()
        if not answer:
            sys.exit(1)
...
def main():
    ...
    init_parser = subparsers.add_parser(
        "init", help="Initialize a new datashelf directory"
    )
    init_parser.add_argument(
        "--path",
        help="Directory to initialize datashelf in (default: current directory)",
    )
    init_parser.set_defaults(func=init_command)

    # Create collection command
    create_parser = subparsers.add_parser(
        "create-collection", help="Create a new collection in .datashelf/"
    )
    ...
    ls_parser = subparsers.add_parser(
        "ls", help="Display metadata of .datashelf/ or specific collections"
    )
    ...
    save_parser = subparsers.add_parser(
        "save", help="Save a file to a collection in .datashelf/"
    )
    save_parser.add_argument("--file_path", help="Path of the file you want to save")
    save_parser.add_argument(
        "--collection_name", help="Name of collection you want to save file into"
    )
    ...
    save_parser.add_argument(
        ...
        "-d",
        action="store_true",
        help="Include if you want file to be duplicated to .datashelf/ instead of moved.",
    )
    ...
    checkout_parser = subparsers.add_parser(
        "checkout",
        help="Checkout a dataset from your collection. Data set will be copied to the same directory that .datashelf/ is in.",
    )
    checkout_parser.add_argument(
        "collection_name", help="Select the collection to checkout from."
    )
    checkout_parser.add_argument(
        "hash", help="Enter the hash of the dataset you want to checkout"
    )
